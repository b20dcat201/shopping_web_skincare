...
// load sản phẩm gợi ý
document.addEventListener("DOMContentLoaded", function () {
    const container = document.getElementById("suggest-product");
    const loadMoreButton = document.getElementById("loadMore");
    const items = container.children;
    const maxItemsToShow = 2;
    let currentItems = maxItemsToShow;

    // Hide items beyond maxItemsToShow
    for (let i = currentItems; i < items.length; i++) {
        items[i].style.display = "none";
    }

    // Show loadMoreButton if there are more than maxItemsToShow
    if (items.length > maxItemsToShow) {
        loadMoreButton.style.display = "block";
    }

    // Event listener for loadMoreButton
    loadMoreButton.addEventListener("click", function () {
        for (let i = currentItems; i < currentItems + maxItemsToShow; i++) {
            if (items[i]) {
                items[i].style.display = "block";
            }
        }

        currentItems += maxItemsToShow;

        // Hide loadMoreButton if all items are visible
